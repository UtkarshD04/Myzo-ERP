...
// an IT department Admin to approve before the actual checkout is recorded.
// This collection tracks that request lifecycle. Field Employees are exempt
// from this gate; see autoPunchOutOpenRecords instead.
...
// Requester sees their own; Admin/HR get full visibility for oversight, but
// only an IT department Admin can actually act on one (see
// attendanceService.reviewLateCheckoutRequest).
export function filterLateCheckoutRequestsForViewer(requests, viewer) {
  if (['Admin', 'HR'].includes(viewer.role)) return requests;
  return requests.filter(r => r.employeeId === viewer.id);
}
